...
mapfile -t containers < <(docker ps -a --format "{{.ID}} {{.Names}} {{.Ports}}")
...
# 读取用户输入
read -p "请输入要删除的容器编号: " input_index
...
if [[ -n "$pid_to_stop" ]]; then
    # 获取容器ID
    container_id=$(docker inspect -f '{{.Id}}' "$name_to_stop" 2>/dev/null)

    if [[ -n "$container_id" ]]; then
        # 判断容器是否正在运行
        if [[ $(docker inspect -f '{{.State.Running}}' "$container_id") == "true" ]]; then
            echo -e "${RED}容器 $name_to_stop 正在运行，请先停止容器。${NC}"
            read -p "按任意键继续..." -n1 -s
            exit 1
        fi
        read -p "确认要删除容器 $name_to_stop 吗？(y/n): " confirm
        if [[ $confirm == [Yy] ]]; then
            docker rm "$container_id" && echo -e "${GREEN}容器已删除：$name_to_stop${NC}" || echo -e "${RED}删除失败。${NC}"
        else
